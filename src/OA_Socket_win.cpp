...
int OpenAPIxx::Socket::send(
	const char* buf, unsigned int length, unsigned int* sentLength)
{
...
int OpenAPIxx::Socket::recv(
	char* buf, unsigned int bufSize, unsigned int* receivedLength)
{
...
int OpenAPIxx::Socket::setNoneBlockModel(bool flag)
{
	...
	if (flag == true)
		...
		value = 0;
	ret = ioctlsocket(m_socketFd, FIONBIO, &value);
	if (ret == -1)
	{
		m_errno = WSAGetLastError();
		return OA_ERR_SYSTEM_CALL_FAILED;
...
int OpenAPIxx::Socket::getSocketName(unsigned int & port)
{
    struct sockaddr_in addr;
